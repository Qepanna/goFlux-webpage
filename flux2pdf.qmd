...
# CH4
CH4_flux <- goFlux(LGR_manID, "CH4dry_ppm")
CH4_flux_res <- best.flux(CH4_flux, criteria = c("MAE", "AICc", "g.factor", "MDF"))
CH4_flux_plots <- flux.plot(CH4_flux_res, LGR_manID, "CH4dry_ppm", shoulder=20,
                            plot.legend = c("MAE", "RMSE", "AICc", "k.ratio", "g.factor"), 
                            plot.display = c("MDF", "prec", "nb.obs", "flux.term"), 
                            quality.check = TRUE)
```
...
CO2_flux_res <- best.flux(CO2_flux, criteria)
CH4_flux_res <- best.flux(CH4_flux, criteria)
...
plot.list <- c(CO2_flux_plots, CH4_flux_plots)
```
:::
