...
-   [**LI-COR**](import.qmd#sec-LICOR-loop): LI-6400, LI-7810, LI-7820, LI-8100, LI-8200 (smart chamber)
...
-   [**Picarro**](import.qmd#sec-picarro-loop): G2508 and G4301
-   [**Gasmet**](import.qmd#sec-gasmet-loop): DX4015
...
| `instrument`      | character string; specifies which instrument was used to generate the files contained in the folder path. Choose one of the following: "DX4015", "EGM5", "G2508", "G4301" "GAIA", "LI-6400", "LI-7810", "LI-7820", "LI-8100", "LI-8200", "LGR" or "N2OM1". For more information about an instrument, see the section "See Also" below. |
...
| `proc.data.field` | numeric value; select the process data field used with the EGM-5. There are 5 different modes available: (1) Measure mode; (2) SRC or Custom mode; (3) CPY mode; (4) Injection mode; or (5) Static mode. If no value specified, the parameters will be named "param1", "param2", "param3", "param4" and "param5".                      |
...
-   [**LI-COR**](import.qmd#sec-LICOR-single): LI-6400, LI-7810, LI-7820, LI-8100, LI-8200 (smart chamber)
...
-   [**Gasmet**](import.qmd#sec-gasmet-single): DX4015
...
Import single raw gas measurement files from the Gasmet DX4015 gas analyzer (CO, CO~2~, CH~4~, N~2~O, NH~3~ and H~2~O) with the function `DX4015_import`.
...
DX4015_import(inputfile = file.path, save = TRUE)
```
...
Import single raw gas measurement files from the PP-Systems EGM-5 Portable CO~2~ Gas Analyzer, with the function `EGM5_import`.
...
# Or save in a RData folder in the working directory
EGM5_import(inputfile = file.path, save = TRUE)
```
...
Import single raw gas measurement files from the Picarro G2508 (CO~2~, CH~4~, N~2~O, NH~3~, and H~2~O GHG analyzer) with the function `G2508_import`.
...
Import single raw gas measurement files from the Picarro G4301 (CO~2~, CH~4~ and H~2~O GHG analyzer) with the function `G4301_import`.
...
# Or save in a RData folder in the working directory
G4301_import(inputfile = file.path, save = TRUE)
```

## GAIATECH {#sec-GAIA-single}

### Automated chamber ECOFlux

Import single raw gas measurement files from the automated chamber ECOFlux (GAIA2TECH) (LI-7810: CO~2~, CH~4~ and H~2~O / LI7820: N~2~O and H~2~O) with the function `GAIA_import`.
...
Import single raw gas measurement files from the ultra-portable GHG analyzers (GLA132-UGGA and GLA131-MGGA) from Los Gatos Research (CO~2~, CH~4~ and H~2~O) with the function `LGR_import`.
...
# Import in the environment
LGR_imp <- LGR_import(inputfile = file.path, prec = c(0.2, 1.4, 50))

# Or save in a RData folder in the working directory
LGR_import(inputfile = file.path, prec = c(0.2, 1.4, 50), save = TRUE)
```
...
# Import in the environment
LGR_imp <- LGR_import(inputfile = file.path, prec = c(0.35, 0.9, 200))

# Or save in a RData folder in the working directory
LGR_import(inputfile = file.path, prec = c(0.35, 0.9, 200), save = TRUE)
```
...
# Import in the environment
N2OM1_imp <- N2OM1_import(inputfile = file.path, prec = c(0.5, 2, 50))

# Or save in a RData folder in the working directory
N2OM1_import(inputfile = file.path, prec = c(0.5, 2, 50), save = TRUE)
```
...
Import single raw gas measurement files from the LI-COR 6400 (CO~2~ and H~2~O GHG analyzer) with the function `LI6400_import`.
...
Import single raw gas measurement files from the LI-COR 7810 (CO~2~, CH~4~ and H~2~O GHG analyzer) with the function `LI7810_import`.
...
Import single raw gas measurement files from the LI-COR 7820 (N~2~O and H~2~O GHG analyzer) with the function `LI7820_import`.
...
Import single raw gas measurement files from the LI-COR 8100 (CO~2~ and H~2~O GHG analyzer) with the function `LI8100_import`.
...
Import single raw data files from the LI-COR smart chamber (LI-8200) with the function `LI8200_import`.
...
#| eval: false
#| code-copy: false
LI8200_import(inputfile, date.format = "ymd", timezone = "UTC", save = FALSE)
```
...
Import single raw data files from the MIRA Ultra CH~4~/ C~2~H~6~ Natural Gas Leak Detection System (with GPS) from Aeris Technologies with the function `uCH4_import`.
...
# Or save in a RData folder in the working directory
uCH4_import(inputfile = file.path, save = TRUE)
```

### MIRA Ultra N~2~O/ CO~2~ analyzer

Import single raw data files from the MIRA Ultra N~2~O/ CO~2~ High Accuracy Analyzer (with GPS) from Aeris Technologies with the function `uN2O_import`.
...
# Or save in a RData folder in the working directory
uN2O_import(inputfile = file.path, save = TRUE)
```
